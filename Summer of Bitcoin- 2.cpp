...
string input = "mempool - mempool.csv";   // Reference to file where dataset is available
...
int main()
{
    ...
    int finalBlockWeight = 0;
    int totalFee = 0;

    while(!tx_set.empty() && finalBlockWeight < totalBlockWeight)
    {
        bool found = false;
        for(auto itr = tx_set.begin(); itr != tx_set.end(); itr++)
        {
            ...
            if(isValidTx(curr_tx, included_tx_set) && finalBlockWeight + currWeight <= totalBlockWeight)
            {
                finalBlockWeight += currWeight;
    ...
    writeOutput(included_tx_vector, "output2.txt");
